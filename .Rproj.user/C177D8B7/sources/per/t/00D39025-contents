...
for (i in vars){
  ...
  hist(df[[i]], main = i, xlab = i, col = coul )
...
vars <- c("HealthGen","SleepTrouble","Weight_factor","Race1", "Gender", "Old","Work")
for (i in vars){
  coul <- brewer.pal(5, "Set2")
  barplot(prop.table(table(df[[i]])),cex.names=0.75, main = i, ylab = "Frequency",col=coul)
...
chi_test <- chisq.test(table, simulate.p.value = TRUE)
print(chi_test)

table_2 <- table(df$SleepTrouble, df$Weight_factor)
chi_test_2 <- chisq.test(table_2, simulate.p.value = TRUE)
print(chi_test)
...
#4) Logistic regression and AUC for prediction of binary health group from sleep trouble and hours slept per night. 
